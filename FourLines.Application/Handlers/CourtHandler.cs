...
public class CourtHandler(FourLinesContext context)
{
    ...
    public async Task<Result<Court>> Create(CreateCourtDTO newCourt)
    {
        Facility? facility = await _context.Facilities
            .FirstOrDefaultAsync(f => f.Id == newCourt.FacilityId && f.OwnerId == newCourt.OwnerId);
        if (facility is null)
    ...
    {
        Facility? facility = await _context.Facilities.FirstOrDefaultAsync(f => f.Id == court.FacilityId);
        if (facility is null)
    ...
    public async Task<Result<bool>> Delete(Guid courtId, Guid facilityId, Guid ownerId)
    {
        bool deleted = false;
        int affectedRows = await _context.Courts
            .Where(c => c.Id == courtId && c.Facility.Id == facilityId && c.Facility.OwnerId == ownerId)
            .ExecuteDeleteAsync();
    ...
    public async Task<Result<Court>> GetFacility(Guid ownerId, Guid facilityId, Guid courtId)
    {
        Court? court = await _context.Courts
            .FirstOrDefaultAsync(c => c.Id == courtId && c.Facility.Id == facilityId && c.Facility.OwnerId == ownerId);
    ...
    public async Task<Result<IEnumerable<Court>>> GetAllCourtsFromFacility(Guid ownerId, Guid facilityId)
    {
        ...
        if (courts is null || !courts.Any())
            return Result<IEnumerable<Court>>.Failure(CourtsErrorResults.RetrieveGetCourtDoesNotExist);
